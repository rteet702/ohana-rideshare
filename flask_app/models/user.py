...
class User:
    ...
    def validate_registration(form:dict) -> bool:
        ...
        if not EMAIL_REGEX.match(form.get('email_registration')):
            flash('* Invalid not valid', 'registration')
            is_valid = False
        ...
        if not PASSWORD_REGEX.match(form.get('password_registration')):
            flash('* Invalid password, must be at least 8 characters.', 'registration')
            is_valid = False
    ...
    def get_by_id(cls, data:dict):
        """Check if there is an user associated with the provided id. If a user is found, returns an object constructed from the query."""
